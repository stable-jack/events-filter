...
export class EventTracker {
  ...
  async initialize() {
    ...
    if (cachedBlock) {
      this.latestProcessedBlock = Math.max(parseInt(cachedBlock, 10), config.startBlockNumber);
  ...
  private async collectPastEvents() {
    ...
    for (let fromBlock = this.latestProcessedBlock; fromBlock <= latestBlock; fromBlock += batchSize) {
  ...
  private startListening() {
    ...
    this.provider.on('block', async (blockNumber) => {
      ...
      if (blockNumber > this.latestProcessedBlock) {
        await this.processBlockRange(this.latestProcessedBlock + 1, blockNumber);
    ...
    this.pollingInterval = setInterval(async () => {
      try {
        ...
        if (latestBlock > this.latestProcessedBlock) {
          console.log(`Polling: Processing blocks ${this.latestProcessedBlock + 1} to ${latestBlock}`);
          await this.processBlockRange(this.latestProcessedBlock + 1, latestBlock);
          await this.updateLatestProcessedBlock(latestBlock);
        }
      } catch (error) {
  ...
  private startRescanning() {
    ...
    this.rescanInterval = setInterval(async () => {
      try {
        ...
        const fromBlock = Math.max(this.latestProcessedBlock - rescanBlockRange, 0);
        console.log(`Rescanning: Processing blocks ${fromBlock} to ${this.latestProcessedBlock}`);
  ...
  private async processBlockRange(fromBlock: number, toBlock: number) {
    ...
    console.log(`Found ${logs.length} events in block range ${fromBlock}-${toBlock}`);
  ...
  private async processEvent(log: Log) {
    ...
    if (!existingEvent) {
      let parsedLog;
      try {
        parsedLog = this.iface.parseLog({ topics: Array.from(log.topics), data: log.data });
      } catch (error) {
      ...
      try {
        ...
        console.log(`Processed new event: ${newEvent.eventName} (Block: ${newEvent.blockNumber}, Tx: ${newEvent.transactionHash})`);
      } catch (error) {
        if (error.code === '23505') {
          console.log(`Duplicate event detected: ${newEvent.transactionHash} - ${newEvent.logIndex}`);
        } else {
          throw error;
        }
      }
